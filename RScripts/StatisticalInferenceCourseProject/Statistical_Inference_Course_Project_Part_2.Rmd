...
qplot(value, len, data = long) + facet_wrap(~ variable, scales = "free")+ggtitle("Tooth Growth vs Supp and Tooth Growth vs Dose")+xlab("")+ylab("Tooth Length")
qplot(dose, len, data=ToothGrowth)+facet_wrap(~ supp)+ggtitle("Tooth Growth vs Dose by Supp")+xlab("Supplement")+ylab("Tooth Length")
```

From the graphs, it seems like there is a positive correlation between dosage and tooth growth but it is not evident when it comes to supplements. This is further confirmed when we break it down by supplement to remove their effects. The graphs show that higher dosage leads to more tooth growth, regardless of which supplement is used.
...
t.test(len_oj, len_vc, alternative = c("two.sided"), conf.level = .95, paired=FALSE, var.equal = FALSE)
```
...
t.test(len_1.0, len_0.5, alternative = c("two.sided"), conf.level = .95, paired=FALSE, var.equal = FALSE)
```
...
t.test(len_2.0, len_1.0, alternative = c("two.sided"), conf.level = .95, paired=FALSE, var.equal = FALSE)
```
